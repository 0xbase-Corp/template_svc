...
var PortfolioRoutes = func(router *gin.Engine, db *gorm.DB) {
	...
	v1.POST("/bundles", func(c *gin.Context) { controllers.CreateBundleController(c, db) })

	v1.GET("/bundles/:id", func(c *gin.Context) { controllers.GetBundlesByIdController(c, db) })
