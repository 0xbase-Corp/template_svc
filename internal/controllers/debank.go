...
// @Description  Retrieves information for a given Debank Wallet address.
// @Tags         debank
// @Accept       json
// @Produce      json
...
func DebankController(c *gin.Context, db *gorm.DB, apiClient providers.APIClient) {
	...
	walletsAddresses := strings.Split(addresses, ",")

	walletsAddresses = utils.UniqueAddress(walletsAddresses)
	...
	wg := &sync.WaitGroup{}
	mutex := &sync.Mutex{}
	ch := make(chan *models.GlobalWallet, len(walletsAddresses)) // len(walletsAddresses) specifies the buffer size of the channel
	errorCh := make(chan error, len(walletsAddresses))

	for _, walletAddress := range walletsAddresses {
		wg.Add(1)

		go fetchAndSaveDebank(db, apiClient, walletAddress, ch, wg, mutex, errorCh)
	}
