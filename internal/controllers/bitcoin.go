...
// @Description  Retrieves information for a given Bitcoin Wallet address using the BTC.com API.
// @Tags         bitcoin
// @Accept       json
// @Produce      json
// @Param        addresses  query      array  true  "Wallet Addresses" Format(string)
...
func BitcoinController(c *gin.Context, db *gorm.DB, apiClient providers.APIClient) {
	...
	walletAddresses := strings.Split(addresses, ",")

	walletAddresses = utils.UniqueAddress(walletAddresses)
	...
	wg := &sync.WaitGroup{}
	mutex := &sync.Mutex{}
	ch := make(chan *models.GlobalWallet, len(walletAddresses)) // len(walletAddresses) specifies the buffer size of the channel
	errorCh := make(chan error, len(walletAddresses))

	for _, walletAddress := range walletAddresses {
		wg.Add(1)

		go fetchAndSaveBtc(db, apiClient, walletAddress, ch, wg, mutex, errorCh)
	}
